...
## Применение в repository (состояние на PHASE 00)

| Правило                                  | Как соблюдается сейчас                                                                                                                                                                                                                                                                    |
| ---------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.1 Source of Truth                      | Бизнес-состояния ещё нет. PostgreSQL поднят локально (`infra/docker/compose.yaml`) и станет authoritative store в PHASE 02.                                                                                                                                                               |
| 2.2 Event History                        | Не реализовано (PHASE 03). Пакет `@roi-dealer/events` — placeholder.                                                                                                                                                                                                                      |
...
| 2.11 Security                            | `.env` в `.gitignore`; `.env.example` без секретов; ошибки конфигурации не содержат значений; logger редактирует чувствительные ключи; `/health` не раскрывает текст ошибок; PostgreSQL слушает только `127.0.0.1`; lifecycle-скрипты зависимостей заблокированы (`pnpm-workspace.yaml`). |
| 2.12 No silent architecture changes      | Процесс ADR — [`docs/ADR/`](ADR/README.md). Решения PHASE 00 — ADR-0001 (Proposed).                                                                                                                                                                                                       |
| 2.13 Retrieval Before Reasoning          | Применимо с появлением authoritative data (PHASE 02+).                                                                                                                                                                                                                                    |
