...
## Применение в repository (состояние на 13b)
...
| 2.6 Ограниченные автоматизации           | Workflows отсутствуют. Стоп-кран (13b): сущность `SystemControl`, `/stop` и `/resume` в пульте; каждая будущая автоматизация первым шагом вызывает `assertAutomationRunning`. Graceful shutdown имеет timeout (10 s).                                                                                                                                                                                                                                   |
| 2.11 Security                            | `.env` в `.gitignore`; `.env.example` без секретов; ошибки конфигурации не содержат значений; logger редактирует чувствительные ключи; `/health` не раскрывает текст ошибок; PostgreSQL слушает только `127.0.0.1`; lifecycle-скрипты зависимостей заблокированы (`pnpm-workspace.yaml`); события — аудит изменений с актором и `correlation_id` (PHASE 03); решения из Telegram — только владелец, подтверждение кнопкой, идемпотентная команда (13b). |
