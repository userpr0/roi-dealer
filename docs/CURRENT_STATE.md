...
```text
Current Phase: 02 — PostgreSQL Foundation
Phase Status: PASS
Completed: 00 — Project Bootstrap (PASS), 01 — Core Domain (PASS, accepted D-015), 13a — Telegram Owner Bot (PASS, owner-approved early step)
Playbook: v1.1
Updated: 2026-09-25
```

Отчёты: [PHASE 00](phases/00_project_bootstrap.report.md), [13a — Telegram Owner Bot](phases/13a_telegram_owner_bot.report.md), [PHASE 01 — Core Domain](phases/01_core_domain.report.md), [PHASE 02 — PostgreSQL Foundation](phases/02_postgresql_foundation.report.md)
...
- Vitest: 32 test files, 327 tests (256 unit, 71 integration); integration-тесты работают с настоящим PostgreSQL 18.
...
- Документация: конституция, архитектура, стек, протокол, ADR (ADR-0001…0004 Accepted, ADR-0005 Proposed), решения владельца (D-001…D-015), план кнопок пульта, phases, runbooks, playbook v1.1.
...
| `@roi-dealer/database`                                                                   | Реализован (PHASE 02): подключение в UTC, миграции, 13 репозиториев, health check |
...
- Схема: миграция `database/migrations/0001_core_domain.sql` (`pnpm db:migrate`); integration-тесты создают на сервере временные БД и удаляют их.
...
| ----------- | ----- | ----- | --------- |
| unit        | 24    | 256   | pass      |
| integration | 8     | 71    | pass      |

Integration-тесты запускают реальные процессы api / worker / bot (SIGTERM, SIGINT, exit codes, некорректная конфигурация), реальный HTTP-сервер, поддельный Telegram Bot API и PostgreSQL 18: миграции, все репозитории, ограничения БД в обход домена и полный цикл DoD v0.1 с сохранением.
...
**03 (Event History) only after owner approval.** Затем 13b — одобрения и стоп-кран в пульте, вместе с облачной PostgreSQL и staging (D-002). ADR-0005 принимается вместе с приёмкой PHASE 02.
